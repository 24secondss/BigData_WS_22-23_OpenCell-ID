...
app.get('/:longitude/:latitude/:radio', function(req, res) {
    let sqlQueryStatement = `SELECT AVG(partition_${req.params.radio}.range) as avgRange FROM partition_${req.params.radio} WHERE lon <= (${req.params.longitude} + 1) AND lon >= (${req.params.longitude} - 1) AND lat <= (${req.params.latitude} + 1) AND lat >= (${req.params.latitude} - 1);`; 
