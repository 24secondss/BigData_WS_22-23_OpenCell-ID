...
app.get('/:longitude/:latitude/:radio', function(req, res) {
    let sqlQueryStatement = `SELECT partition_${req.params.radio}.range FROM partition_${req.params.radio} WHERE lon <= (${req.params.longitude} + 1) AND lon >= (${req.params.longitude} - 1) AND lat <= (${req.params.latitude} + 1) AND lat >= (${req.params.latitude} - 1) LIMIT 1;`; 
